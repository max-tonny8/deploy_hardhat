...
error Whitelist_alreadyExist(uint256 _address, string message);
error Whitelist_limitOverflow(string message);

contract Whitelist {

   event addressAdded(
    uint256 indexed whitelistAddress
   ); 
    ...
    function addAddressToWhiteList(uint256 _address) public {
        if(s_maxWhitelistAddresses[msg.sender])
          revert Whitelist_alreadyExist(_address: msg.sender, message: "Sender has already been whitelisted");
        if(s_addressesListedCount >= s_maxWhitelistAddresses )
          revert Whitelist_limitOverflow(message: "More addresses cant be added, limit reached");
